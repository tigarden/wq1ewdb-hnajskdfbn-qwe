...
async def import_database(payload: FullBackupPayload, db: AsyncSession = Depends(get_db)):
    """Import and synchronize JSON data into PostgreSQL atomically."""
    try:
        # 1. Clients
        for cli_dict in payload.clients:
            ...
            if existing:
                ...
                existing.initial_balance = float(cli_dict.get("initialBalance", existing.initial_balance) or 0.0)
                ...
                db.add(Client(
                    ...
                    initial_balance=float(cli_dict.get("initialBalance", 0.0) or 0.0),
        ...
        for tx_dict in payload.clientTransactions:
            ...
            if not existing_tx:
                db.add(ClientTransaction(
                    ...
                    article=tx_dict.get("article", ""),
                    description=tx_dict.get("description", ""),
                    car_name=tx_dict.get("carName", ""),
                    supplier_name=tx_dict.get("supplierName", ""),
                    amount=float(tx_dict.get("amount", 0.0) or 0.0),
                    purchase_price=float(tx_dict.get("purchasePrice", 0.0) or 0.0),
        ...
        for ot_dict in payload.otherTransactions:
            ...
            if not existing_ot:
                db.add(OtherTransaction(
                    ...
                    amount=float(ot_dict.get("amount", 0.0) or 0.0),
                    note=ot_dict.get("note", ""),
                    date=ot_dict.get("date", ""),
                ))

        await db.commit()
        return {"success": True, "message": "Данные успешно импортированы в базу данных"}
