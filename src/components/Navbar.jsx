      ...
      <header className="sticky top-0 z-40 bg-[#080b12]/90 backdrop-blur-xl border-b border-white/[0.08] shadow-xs">
        <div className="max-w-[1680px] 2xl:max-w-[1880px] mx-auto px-3 sm:px-6 lg:px-8 2xl:px-10">
          <div className="flex items-center justify-between h-14 2xl:h-16">
            ...
            >
              <div className="w-7 h-7 2xl:w-8 2xl:h-8 rounded-lg bg-gradient-to-tr from-blue-700 to-blue-500 flex items-center justify-center text-white font-black text-xs 2xl:text-sm shadow-md shadow-blue-500/25 border border-white/20 group-hover:scale-105 transition-transform">
                D
              </div>
              <div className="flex items-center space-x-2">
                <span className="text-sm 2xl:text-base font-bold tracking-tight text-white uppercase font-mono">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center">
                {syncStatus === 'syncing' ? (
                  <div className="h-8 2xl:h-9 px-2.5 2xl:px-3 rounded-lg bg-amber-500/10 text-amber-300 border border-amber-500/25 text-xs 2xl:text-sm font-medium inline-flex items-center space-x-1.5">
                    <RefreshCw className="w-3.5 h-3.5 animate-spin text-amber-400" />
                    <span className="hidden sm:inline font-mono">Обновление...</span>
                  </div>
                ) : syncStatus === 'synced' ? (
                  <button 
                    onClick={() => pushToGitHub()} 
                    title={`Синхронизировано: ${lastSyncTime ? new Date(lastSyncTime).toLocaleTimeString('ru-RU') : 'только что'}`}
                    className="h-8 2xl:h-9 px-2.5 2xl:px-3 rounded-lg bg-emerald-500/10 text-emerald-300 border border-emerald-500/25 text-xs 2xl:text-sm font-medium hover:bg-emerald-500/20 transition-colors inline-flex items-center space-x-1.5 font-mono"
                  >
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse-dot" />
                    <Cloud className="w-3.5 h-3.5 2xl:w-4 2xl:h-4 text-emerald-400" />
                    <span className="hidden sm:inline">Синхронно</span>
                  </button>
                ) : syncStatus === 'unsaved' ? (
                  <button
                    onClick={() => pushToGitHub()}
                    className="h-8 2xl:h-9 px-2.5 2xl:px-3 rounded-lg bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-bold text-xs 2xl:text-sm inline-flex items-center space-x-1.5 shadow-sm shadow-amber-500/20 active:scale-95 transition-transform"
                    title="Есть несохраненные данные в облако"
                  >
                    <Cloud className="w-3.5 h-3.5 2xl:w-4 2xl:h-4" />
                    <span>Сохранить</span>
                  </button>
                ) : (
                  <button 
                    onClick={() => setActiveTab('settings')}
                    className="h-8 2xl:h-9 px-2.5 2xl:px-3 rounded-lg bg-slate-950 text-slate-400 border border-white/10 text-xs 2xl:text-sm font-medium hover:text-slate-200 transition-colors inline-flex items-center space-x-1.5"
                    title="Оффлайн режим"
                  >
                    <CloudOff className="w-3.5 h-3.5 2xl:w-4 2xl:h-4 text-slate-500" />
              ...
              <button
                ...
                className="h-8 2xl:h-9 px-2.5 2xl:px-3 rounded-lg text-slate-300 hover:text-white bg-slate-950/80 hover:bg-slate-900 border border-white/10 text-xs 2xl:text-sm font-medium inline-flex items-center space-x-1.5 transition-colors"
              >
              ...
              {!isStandalone && onOpenInstallModal && (
                <button
                  ...
                  className="h-8 2xl:h-9 px-2.5 2xl:px-3 rounded-lg text-blue-400 hover:text-white bg-blue-500/10 hover:bg-blue-600 border border-blue-500/25 text-xs 2xl:text-sm font-semibold inline-flex items-center space-x-1.5 transition-colors"
                >
              ...
              <button
                ...
                className="h-8 2xl:h-9 w-8 2xl:w-9 rounded-lg text-slate-400 hover:text-rose-400 bg-slate-950/80 hover:bg-slate-900 border border-white/10 flex items-center justify-center transition-colors"
              >
                <Lock className="w-3.5 h-3.5 2xl:w-4 2xl:h-4" />
              </button>

              {/* Primary Quick Add Button (Desktop) */}
              <button
                onClick={onOpenQuickAdd}
                className="hidden sm:inline-flex btn-sm 2xl:btn-md btn-primary font-bold shadow-md shadow-blue-500/25"
              >
